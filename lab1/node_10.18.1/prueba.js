...
function test() {
  ...
  async function getBucketName() {
    var buckets_arr = (await S3_API.listBuckets().promise()).Buckets;
    for (var i_int = 0; i_int < buckets_arr.length; i_int += 1) {
      if (buckets_arr[i_int].Name.indexOf("fb-s3site") !== -1) {
  ...
  function createWorkableDataStructure() {
    var final_arr = [],
      merged_construct = {};
    for (var i_int = 0; i_int < g_sentiment_data_arr.length; i_int += 1) {
      merged_construct = {}; //reset
      //product info
      console.log(g_tag_data_arr.length, g_sentiment_data_arr.length);
      merged_construct.product_id_str =
        g_sentiment_data_arr[i_int].product_info.product_id;
      merged_construct.review_headline_str =
        g_sentiment_data_arr[i_int].product_info.review_headline;
      merged_construct.review_body_str =
        g_sentiment_data_arr[i_int].product_info.review_body;
      merged_construct.star_rating_float = Number(
        g_sentiment_data_arr[i_int].product_info.star_rating
      );
      //sentiment result
      merged_construct.sentiment_str =
        g_sentiment_data_arr[i_int].Sentiment.toLowerCase();
      ...
      for (
        var j_int = 0;
        j_int < g_tag_data_arr[i_int].Entities.length;
        j_int += 1
      ) {
        // console.log(g_tag_data_arr[i_int].Entities[j_int].Type);
        if (
          merged_construct.entity_str_arr.indexOf(
            g_tag_data_arr[i_int].Entities[j_int].Type.toLowerCase()
          ) !== -1
        ) {
          //skip no dups
        } else {
          merged_construct.entity_str_arr.push(
            g_tag_data_arr[i_int].Entities[j_int].Type.toLowerCase()
          );
        }
  ...
  function pushJSONToTemplate() {
    ...
    for (var i_int = 0; i_int < g_workable_data_arr.length; i_int += 1) {
      html_str += "<div>";
      html_str +=
        "<span>" + g_workable_data_arr[i_int].product_id_str + "</span>";
      html_str +=
        "<h2>" + g_workable_data_arr[i_int].review_headline_str + "</h2>";
      html_str += "<h3>" + g_workable_data_arr[i_int].sentiment_str + "</h3>";
      html_str +=
        "<h4>" +
        g_workable_data_arr[i_int].star_rating_float.toString() +
        "</h4>";
      if (g_workable_data_arr[i_int].entity_str_arr) {
        html_str += "<section>";
        for (
          var j_int = 0;
          j_int < g_workable_data_arr[i_int].entity_str_arr.length;
          j_int += 1
        ) {
          html_str += "<span>";
